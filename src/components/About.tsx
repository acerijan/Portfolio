...
export function SectionHeading({ title }: { title: string }) {
  ...
  return (
    <div className="flex items-center gap-3 mb-10">
      <h2 className={`text-2xl font-bold transition-colors duration-700 ${isNight ? 'text-white' : 'text-slate-800'
        }`}>
        {title}
      </h2>
      <div className={`flex-1 h-px max-w-[200px] transition-colors duration-700 ${isNight ? 'bg-white/5' : 'bg-slate-200'
        }`} />
    </div>
  )
}
  ...
  return (
    <section id="about" className="py-20 md:py-28 px-5">
      <div className="max-w-5xl mx-auto">
        ...
        <div className={`max-w-2xl space-y-4 leading-relaxed transition-colors duration-700 ${isNight ? 'text-slate-400' : 'text-slate-600'
          }`}>
          <p>
          ...
          <p>
            When I'm not coding, you'll find me playing soothing music,
